...
MainWindow::MainWindow():m_bEdited(false){
    QHBoxLayout *lyt1=new QHBoxLayout;lyt1->setContentsMargins(0,0,0,0);Common *c=Common::i();
    StatusBar *statusBar=new StatusBar(true);connect(c,&Common::showMessage,statusBar,&StatusBar::showMessage,Qt::QueuedConnection);

    lyt1->addWidget(new MenuBar);lyt1->addWidget(statusBar,1);
    ...
    QVBoxLayout *lyt=new QVBoxLayout;lyt->setContentsMargins(0,0,0,0);lyt->setSpacing(1);
    lyt->addLayout(lyt1);lyt->addWidget(splitter,1);
    QWidget *w=new QWidget;w->setLayout(lyt);setCentralWidget(w);

    m_title=c->p_softwareName;updateTitle();
    connect(c,&Common::setEditedState,this,&MainWindow::setEditedState,Qt::QueuedConnection);
    connect(c,&Common::setProjectFileName,this,&MainWindow::setProjectFileName,Qt::QueuedConnection);
    connect(c,&Common::setProjectStatus,this,[this](QString status){m_status=status;updateTitle();},Qt::QueuedConnection);
