...
void ContrastAdjuster::initUI(){
    QHBoxLayout *toolLayout=new QHBoxLayout;setLayout(toolLayout);
    toolLayout->setContentsMargins(0,0,0,0);toolLayout->setSpacing(1);

    m_spinbox=new QSpinBox;m_spinbox->setMaximumWidth(70);
    ...
    toolLayout->addWidget(m_slider);toolLayout->addWidget(m_spinbox);
...
void ContrastAdjuster::updateSlider(int v){
    ...
    if(v<m_spinbox->minimum()){v=m_spinbox->minimum();}
    if(v>m_spinbox->maximum()){v=m_spinbox->maximum();}
...
void ContrastAdjuster::updateSpinbox(int v){
    ...
    double maxX=m_slider->maximum(),middleV=MiddleRangeFactor*maxX;
    if(v>m_maxValue){
        v=double(v-m_middleValue)/(m_maxValue-m_middleValue)*(maxX-middleV)+middleV;
    }else{
        v=double(v)/m_middleValue*middleV;
    }

    if(v<m_slider->minimum()){v=m_slider->minimum();}
    if(v>m_slider->maximum()){v=m_slider->maximum();}
...
void ContrastAdjuster::viewerCreated(){
    updateSpinbox(m_spinbox->value());
}
