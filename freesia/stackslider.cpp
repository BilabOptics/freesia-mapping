...
inline QToolButton *createBtn(QStyle::StandardPixmap icon) {
  ...
  btn->setIcon(QApplication::style()->standardIcon(icon));  // QIcon(path));
...
StackSlider::StackSlider() : m_fps(8), m_state(States::FREE) {
  ...
  m_btnPause = createBtn(QStyle::SP_MediaStop);
  m_btnPause->hide();
  ...
  lyt->addWidget(m_btnPlay);
  lyt->addWidget(m_btnPause);
  lyt->addWidget(btnBack);
  lyt->addWidget(m_slider);
  lyt->addWidget(btnGo);

  m_indexEdit = new QLineEdit("0");
  m_indexEdit->setMaximumWidth(80);
  m_indexEdit->setObjectName("SliderIndex");
  lyt->addWidget(m_indexEdit);
  m_indexEdit->setValidator(new QIntValidator());
  connect(m_slider, SIGNAL(valueChanged(int)), this,
          SLOT(updateIndexEdit(int)));
  connect(m_indexEdit, SIGNAL(returnPressed()), this, SLOT(indexEditUpdated()));
}

void StackSlider::showPlayButton(bool bVisible) {
  m_btnPlay->setVisible(bVisible);
}

void StackSlider::setValue(int v) {
  if (States::FREE == m_state) {
    return;
  }
...
bool StackSlider::setValueSilent(int v) {
  disconnect(m_slider, SIGNAL(valueChanged(int)), this,
...
void StackSlider::moveSliderRelative(int v) {
  setValueSimple(m_slider->value() + v);
}

void StackSlider::addSlider() {
  if (m_state != States::FREE) {
    return;
  }
  m_state = PRE_INC;
...
void StackSlider::setRange(int minV, int maxV) {
  ...
  QString styleStr = QString("QSlider::handle:horizontal{width:%1px;}")
                         .arg(QString::number(width));
  m_slider->setStyleSheet(styleStr);

  m_indexEdit->setToolTip("from " + QString::number(minV) + " to " +
                          QString::number(maxV));
}

QString StackSlider::text() {
  QString msg = QString("%1/%2").arg(QString::number(m_slider->value()),
                                     QString::number(m_slider->maximum()));

  return msg;
}
int StackSlider::value() { return m_slider->value(); }

void StackSlider::updateIndexEdit(int v) {
  m_indexEdit->setText(QString::number(v));
}
void StackSlider::indexEditUpdated() {
  bool bOK;
  int v = m_indexEdit->text().toInt(&bOK);
  setValueSimple(v);
}
