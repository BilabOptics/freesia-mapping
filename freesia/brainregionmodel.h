...
struct RegionNode {
  int id, parentId, color, level;
  QString acronym, name, path;
  QList<RegionNode *> subRegions;
  RegionNode *parentNode;
  QList<int> voxelIndexes;
  int totalVoxelNumber;

  RegionNode(int _id, int _parentId, QString _acronym, QString _name,
             int _color)
      : id(_id),
        parentId(_parentId),
        acronym(_acronym),
...
class BrainRegionModel : public QObject {
 ...
 public:
  static BrainRegionModel *i() {
    static BrainRegionModel m;
    return &m;
  }

  QString getSelectedModelPath();
