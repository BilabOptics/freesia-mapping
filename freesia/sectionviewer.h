...
class SectionViewer : public QWidget {
  ...
  const int m_type, m_modelFactor;
  int m_groupSize;
  ...
  QMap<int, QList<Node *> *> m_allNodes;
  int m_groupIndex, m_imageIndex2, m_nextGroupIndex, m_nextImageIndex2;
  QList<Node *> *m_nodes;
  Node *m_currentNode, *m_candidateNode, *m_selectNode;

  vtkSmartPointer<vtkRenderer> m_renderer;
  vtkSmartPointer<vtkImageActor> m_imageActors[2], m_selectActors[2],
      m_modelActors[2];
 ...
 public:
  ...
  void getMarkers(int groupIndex, double offsets[2], double scales[2],
                  double rotation, QList<QLineF> &lines);
  QVariantList exportAllMarkers();
  void importAllMarkers(const QVariantList &list);

  bool getUpdatedIndex(int &index);
  bool getImageIndex(int &index);

  void updateResliceImage(const cv::Mat &image, double *origin, double *spacing,
                          double rotation, int imageIndex, int groupIndex);
