...
struct TransformParameters {
  double rotation[3], offset[3], scale[3];
  int sliceIndex;
  TransformParameters() {
    for (int i = 0; i < 3; i++) {
      rotation[i] = 0;
...
class Common : public QObject {
 ...
 public:
  ...
  std::atomic_int p_selectionMode;
  int p_atlasIndex;
