...
BrainRegionPanel::BrainRegionPanel(){
    ...
    setLayout(lyt);lyt->setSpacing(2);

    m_pTableView=new QTreeView;m_pModel=new QStandardItemModel;m_pTableView->setModel(m_pModel);
    ...
    QHBoxLayout *inputLyt=new QHBoxLayout;inputLyt->setContentsMargins(0,0,0,0);
    m_searchEditor=new QLineEdit;QPushButton *btnSearch=new QPushButton("Search");
    m_searchEditor->setPlaceholderText("Enter region name here");
    inputLyt->addWidget(m_searchEditor);inputLyt->addWidget(btnSearch);
    lyt->addLayout(inputLyt);

    loadItems();
    connect(BrainRegionModel::i(),&BrainRegionModel::modelSelected,this,&BrainRegionPanel::loadItems);

    connect(btnSearch,&QPushButton::clicked,this,&BrainRegionPanel::searchRegion);
    connect(m_searchEditor,&QLineEdit::returnPressed,this,&BrainRegionPanel::searchRegion);

    connect(m_pTableView,&QTreeView::clicked,this,&BrainRegionPanel::selectRow);
    connect(Common::i(),&Common::regionSelected2,this,&BrainRegionPanel::selectRegionByColor);
}

QStandardItem *BrainRegionPanel::getItem(const QString &text,const QString &tooltipText){
    QStandardItem *item=new QStandardItem(text);item->setToolTip(tooltipText);
    item->setEditable(false);item->setTextAlignment(Qt::AlignVCenter|Qt::AlignLeft);

    return item;
}
QStandardItem *BrainRegionPanel::getItem(RegionTree *node, QStandardItem *parent){
...
void BrainRegionPanel::searchRegion(){
    ...
    if(name!=m_searchText||m_searchIndex>=m_allRegions.length()){m_searchIndex=0;}
    m_searchText=name;

    for(;m_searchIndex<m_allRegions.length();m_searchIndex++){
        RegionTree *t=m_allRegions[m_searchIndex];
        if(t->acronym.toLower().indexOf(name)>=0||t->name.toLower().indexOf(name)>=0){
            selectRegionByColor(t->color);selectRow(t->modelIndex);break;
        }
    }
...
void BrainRegionPanel::selectRegionByColor(int color){
    RegionTree *node=m_color2RegionTree.value(color);if(nullptr==node){return;}
    QModelIndex index=node->modelIndex;

    m_pTableView->collapseAll();
    QModelIndex parentIndex=index.parent();
    while(parentIndex.isValid()){
        m_pTableView->expand(parentIndex);parentIndex=parentIndex.parent();
    }

    m_pTableView->selectionModel()->select(index,QItemSelectionModel::SelectCurrent);
...
void BrainRegionPanel::selectRow(QModelIndex index){
    ...
    if(nullptr!=node){emit Common::i()->regionSelected(node->color);}
}

void BrainRegionPanel::loadItems(){
    const QString filePath=BrainRegionModel::i()->getSelectedModelPath();
    QVariantMap params;if(!Common::loadJson(filePath,params)){return;}

    qDeleteAll(m_allRegions);m_allRegions.clear();m_rootRegions.clear();m_models2RegionTree.clear();m_id2RegionTree.clear();
    ...
    while(iter1.hasNext()){
        ...
        QString acronym=item["acronym"].toString(),name=item["name"].toString();

        RegionTree *node=new RegionTree(sId,parentId,color,acronym,name);
        m_allRegions.append(node);m_id2RegionTree.insert(sId,node);m_color2RegionTree.insert(color,node);
    }
    QListIterator<RegionTree*> iter2(m_allRegions);
    while(iter2.hasNext()){
        RegionTree *node=iter2.next();int parentId=node->parentId;
        if(!m_id2RegionTree.contains(parentId)){m_rootRegions.append(node);continue;}
